...
describe "Enrollment Requests", :type => :request do
    ...
    describe "PATCH /update" do
        ...
        it "updates a Enrollment with invalid params and redirects to the Enrollment's page" do
            new_params = build(:enrollment, :invalid_params)
    ...
    describe "POST /create" do
        ...
        it "creates a Enrollment with valid params and checks the payments created" do
            ...
            }.to change{Payment.count}.by(params[:number_payments])

            enroll_created = Enrollment.last
            payments = Payment.where(enrollment_id: enroll_created.id).all

            expected_value = enroll_created.course_price / enroll_created.number_payments
            expected_status = 'open'
            current_date = Date.today
            expected_due_date = Date.new(current_date.year, current_date.mon, enroll_created.due_day)

            if enroll_created.due_day <= current_date.mday
